<script setup>
  import { 
    ...
    PhMagnifyingGlassPlus, PhX, PhCamera, PhCrown, PhMusicNotes,
    PhLockKey, PhMagicWand, PhCaretDown
  ...
  const modalRegalosOpen = ref(false) // Alias
...
<template>
  <div class="min-h-screen bg-slate-50 text-slate-800 overflow-x-hidden font-body">
    ...
    <header class="relative min-h-[100dvh] w-full flex flex-col items-center justify-center overflow-hidden bg-[#fafafa]">
      
      <div class="absolute inset-0 z-0">
        <div class="absolute inset-0 bg-white/60 z-10 backdrop-blur-[1px]"></div>
        <img :src="datos.fotoPortada" class="w-full h-full object-cover opacity-80" alt="Fondo" />
        <div class="absolute inset-0 bg-gradient-to-t from-white via-transparent to-white/50 z-10"></div>
      </div>
      ...
      <div class="absolute top-[12vh] z-50 w-full flex justify-center pointer-events-none">
          <transition name="fade" mode="out-in">
            <div v-if="esQueen && nombreInvitado" key="vip" class="animate-fade-in-up pointer-events-auto">
               <div class="bg-gradient-to-r from-rose-300 to-pink-400 text-white px-8 py-2.5 rounded-full shadow-xl border border-white/60 transform hover:scale-105 transition-transform">
                 <p class="font-cinzel font-bold text-xs md:text-sm tracking-[0.2em] uppercase flex items-center gap-2">
                   <PhCrown weight="fill" class="text-yellow-100 text-lg" /> 
                   <span>Para {{ nombreInvitado }}</span>
                 </p>
               </div>
            </div>
            <div v-else-if="!esQueen && esGlow" key="upgrade" class="pointer-events-auto">
               <p class="inline-flex items-center gap-2 text-[10px] md:text-xs font-bold text-rose-500 bg-white/90 backdrop-blur-md px-6 py-2 rounded-full border border-rose-200 shadow-md cursor-pointer hover:scale-105 transition-transform">
                  <PhLockKey weight="fill" class="text-rose-400" />
                  Personaliza con el <span class="text-rose-600 font-black uppercase ml-1">Plan Queen</span>
               </p>
            </div>
          </transition>
      </div>
    
      <div class="relative z-30 w-full max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-center gap-8 md:gap-20 px-6 pt-16">
        
        <div class="relative shrink-0 group z-20 order-1">
            <div class="absolute -inset-4 bg-purple-200/40 rounded-full blur-2xl opacity-60 animate-pulse"></div>
            
            <div class="absolute inset-0 rounded-full border-2 border-dashed border-[#d8b4fe] animate-spin-slow opacity-60 scale-110"></div>
      ...
      <div class="absolute bottom-8 z-30 animate-bounce text-[#75527e]/60 pointer-events-none">
         <div class="flex flex-col items-center gap-2">
            <span class="text-[9px] font-cinzel tracking-[0.2em] uppercase">Desliza</span>
            <PhCaretDown size="20" />
         </div>
      </div>
    ...
    <section v-if="esGlow && datos.galeria" class="py-20 bg-[#75527e] relative overflow-hidden min-h-[600px]">
      
      <div v-if="!esQueen && datos.galeria.length > 6" class="relative z-30 mt-8 mb-8 text-center animate-fade-in-up">
         <p class="text-rose-200 text-xs italic border border-rose-200/30 inline-block px-6 py-2 rounded-full uppercase tracking-wider bg-[#5e3a66]/80 backdrop-blur-md flex items-center justify-center gap-2 shadow-lg cursor-pointer hover:scale-105 transition-transform">
           <PhLockKey weight="fill" class="text-rose-300"/>
           <span>+ {{ datos.galeria.length - 6 }} fotos más en <strong class="text-white font-bold">Plan Queen</strong></span>
         </p>
      </div>
      ...
      <section v-if="esGold" class="bg-white py-10 relative">
        <div class="absolute top-0 left-0 w-full h-20 bg-gradient-to-b from-slate-50 to-white"></div>
          ...
          <div class="text-center mb-16 relative z-10 px-4">
            <h2 class="font-seasons text-4xl md:text-5xl text-[#75527e] mb-2">Itinerario</h2>
            <p class="font-cormorant text-lg md:text-xl text-[#a37ab0] italic">Sigue el ritmo de mi fiesta</p>
          </div>
      ...
      <div class="max-w-lg mx-auto relative z-10">
        <div class="bg-white rounded-[2.5rem] shadow-2xl shadow-black/20 overflow-hidden relative border border-white/20">
          <div class="p-10 md:p-12 text-center space-y-10">
            ...
            <div v-if="datos.regalos && datos.regalos.length > 0" class="w-full flex items-center justify-center opacity-30">
    ...
    <ModalGeneradorLinks 
        ...
        tipoEvento="xv" 
        colorHeader="bg-gradient-to-r from-[#a37ab0] to-[#75527e]" 
...
.mariposa {
  ...
  z-index: 10;
  pointer-events: none; /* Mejora rendimiento */
...
@keyframes vuelo-magico {
  ...
  10% { opacity: 1; }
  25% { transform: translateY(-25vh) translateX(30px) rotate(15deg); }
...
@keyframes brillo-real {
  0% { filter: drop-shadow(0 4px 3px rgba(0,0,0,0.2)) brightness(1); }
  50% { filter: drop-shadow(0 0 8px rgba(255,255,255,0.8)) brightness(1.3); }
  100% { filter: drop-shadow(0 4px 3px rgba(0,0,0,0.2)) brightness(1); }
}
...
.font-seasons { font-family: 'The Seasons', 'Playfair Display', serif; font-weight: 400; }
