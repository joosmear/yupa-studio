<script setup>
  import { PhWhatsappLogo, PhMapPin, PhCalendarCheck, PhGift, PhLeaf } from '@phosphor-icons/vue'
  ...
  onMounted(() => {
    ...
    // Observer para animar iconos al hacer scroll
...
<template>
  <div class="min-h-screen bg-[#FDFBF7] text-[#4A5D4F] overflow-x-hidden font-sans pb-24">
    ...
    <header class="relative min-h-screen flex flex-col justify-end pb-20 px-6 overflow-hidden">
      ...
      <transition name="fade">
        <div v-if="esPremium" class="absolute top-8 left-1/2 -translate-x-1/2 z-20 animate-pulse delay-700">
           <div class="bg-[#4A5D4F]/80 backdrop-blur-md border border-white/20 px-6 py-2 rounded-full shadow-lg">
             <p class="text-[#FDFBF7] text-sm font-bold tracking-wide uppercase text-[10px]">✨ Hola Familia Pérez ✨</p>
           </div>
        </div>
      </transition>
    ...
    <div v-if="!esGold" class="py-12 bg-[#FDFBF7] text-center border-y border-[#4A5D4F]/10">
       <p class="text-[#8A9A5B] text-xs uppercase tracking-widest flex items-center justify-center gap-2 font-bold">
         <PhLeaf weight="fill" /> El Plan Gold incluye Agenda y Detalles
       </p>
    </div>
    ...
    <footer class="bg-[#2C3E30] text-[#E8ECE9] py-12 text-center rounded-t-[3rem] pb-24 ">
       ...
       <div class="max-w-xs mx-auto w-full px-4 mb-8">
           <div class="text-[#8A9A5B] text-[10px] uppercase tracking-widest mb-2 opacity-80">Confirmar antes del {{ fechaLimiteConfirmacion }}</div>
           <BotonAccion 
             ...
             color="bg-[#4A5D4F] text-white hover:bg-[#5e7464]"
           />
       </div>
